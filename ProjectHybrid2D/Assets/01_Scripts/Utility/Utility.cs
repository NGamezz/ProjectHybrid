...
namespace Custom
{
    public class Utility
    {
        ...
        public void Setup ()
        {
            random = new((uint)UnityEngine.Random.Range(1, 100000));
        }
        ...
        public T GetRandomElementFromArray<T> ( List<T> array )
        {
            if ( array.Count < 1 )
            {
                return default;
            ...
            var str = array[random.NextInt(0, array.Count)];
            return str;
        }
        ...
        public T GetRandomElementFromList<T> ( List<T> list, List<T> options, int indexer, int maxIndexer )
        {
